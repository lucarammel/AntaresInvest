...
power_plants = lapply(antares_study_list, function(simulation_name) lapply(areas,function(area) make_powerplants_from_antares(study_id = study_list[[simulation_name]]$variant_id,
                                                                                                                              zone = area, 
                                                                                                                              year_simulated = simulation_name))) %>% rename_dataset(., years_to_simulate = years, 
                                                                                                                                                                                     areas = areas)
...
dataset_revenues = lapply(years, function(year_simulated) lapply(areas, function (area) analysis_revenues_total(year_simulated = year_simulated,
                                                                                                                area = area,
                                                                                                                ...
                                                                                                                data_antares = dataset[[year_simulated]][[area]]$data_antares))) %>% rename_dataset(., years_to_simulate = years, 
                                                                                                                                                                                                    areas = areas)

#---------------------------------------------------- LOAD  --------------------------------------------------#
load = lapply(years,
              function(x) dataset[[x]]$fr$data_antares$areas %>% select(c("mcYear","timeId","LOAD")) %>% 
...
data = lapply(years, function(y) {data[[y]] %>% setNames(.,paste(colnames(data[[y]]), y, sep = '_'))}) %>% setNames(.,years)


df_selection = do.call(cbind, data) 
...
cluster_graph  = function(df,medoids,year="2022", x = "shortage_hour", y = "revenue_mean",xlab="shortage_hour",ylab = "Revenu Moyen"){
  fig = ggplot() + 
...
plot_initial_vs_new_cluster = function(df,initial,year="2022",medoids,x = "shortage_hour", y = "revenue_mean",xlab="shortage_hour",ylab = "Revenu Moyen"){
...
#---------------------------------------------------- 2. Unsupplied energy ----------------------------------------------------#
# We take the same approach on the unsupplied energy side ("shortage_hour")
...
clust_median_only = lapply(names(df_outputs),function(year) make_clustering(df_outputs[[year]],use_median = TRUE)) %>% setNames(.,names(df_outputs))
...
clust_mean_sd = lapply(names(df_outputs),function(year) make_clustering(df_outputs[[year]],use_sd = TRUE)) %>% setNames(.,names(df_outputs))
...
clust_median_sd = lapply(names(df_outputs),function(year) make_clustering(df_outputs[[year]],use_median = TRUE, use_sd = TRUE)) %>% setNames(.,names(df_outputs))
